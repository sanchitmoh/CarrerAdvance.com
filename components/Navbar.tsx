...
const userRoles = [
  { name: 'Teachers', path: '/teachers', icon: '🎓', color: 'from-emerald-500 to-emerald-600' }, // Changed to emerald
  { name: 'Students', path: '/students', icon: '📚', color: 'from-green-500 to-green-600' },     // Changed to green
  { name: 'Employers', path: '/employers', icon: '💼', color: 'from-lime-500 to-lime-600' },     // Changed to lime
  { name: 'Job Seekers', path: '/job-seekers', icon: '🔍', color: 'from-emerald-700 to-green-700' }, // Changed to darker emerald/green
  { name: 'Admin', path: '/admin', icon: '⚡', color: 'from-green-700 to-teal-700' }, // Changed to darker green/teal
]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {['Home', 'Courses', 'Blogs', 'Jobs'].map((item, index) => (
              <div
                key={item}
        ...
        {isOpen && (
          <div
            className="lg:hidden fixed inset-0 pt-20 bg-white/95 backdrop-blur-xl border-b border-gray-200/50 shadow-2xl animate-slide-down overflow-y-auto"
          >
            <div className="px-4 py-8 space-y-6">
              {['Home', 'Courses', 'Blogs', 'Jobs'].map((item, index) => (
                <div
                  key={item}
                  className="opacity-0 translate-x-[-20px] animate-fade-in"
                  style={{ animationDelay: `${index * 0.1}s` }}
                >
                  <Link 
                    href={item === 'Home' ? '/' : `/${item.toLowerCase()}`} 
                    className="block text-gray-700 hover:text-emerald-600 font-semibold py-3 text-lg transition-colors" // Changed to emerald
              ...
              <div className="pt-6 border-t border-gray-200">
                <p className="text-sm font-bold text-gray-900 mb-4">Choose Your Role:</p>
                  ...
                  {userRoles.map((role, index) => (
                    ...
                    >
                      <Link
                        ...
                        className="flex items-center space-x-4 p-4 rounded-xl hover:bg-gradient-to-r hover:from-emerald-50 hover:to-green-50 transition-all duration-300 group" // Changed to emerald/green
                        onClick={() => setIsOpen(false)}
                      >
                        <div className={`w-10 h-10 rounded-xl bg-gradient-to-r ${role.color} flex items-center justify-center text-white shadow-lg`}>
                          <span className="text-lg">{role.icon}</span>
                        </div>
                        <div className="flex-1">
              ...
              <div 
                className="pt-6 space-y-3 opacity-0 animate-fade-in"
                style={{ animationDelay: '0.8s' }}
              >
                <Button className="w-full bg-gradient-to-r from-emerald-500 to-green-600 text-white rounded-xl py-3 font-semibold"> {/* Changed to emerald/green */}
                  Post a Job
                </Button>
                <Button variant="outline" className="w-full border-2 border-emerald-200 text-emerald-600 rounded-xl py-3 font-semibold"> {/* Changed to emerald */}
                  Find Jobs
                </Button>
              </div>
            </div>
