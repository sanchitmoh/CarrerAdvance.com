...
import { FileText, Edit, Save, X, Plus, Award, Sparkles, Loader2 } from 'lucide-react'
...
export default function ProfessionalSummary() {
  ...
  // Load professional summary on mount
  useEffect(() => {
    const loadProfessionalSummary = async () => {
      try {
        ...
        const response = await fetch(
          `/api/seeker/profile/get_professional_summary?jobseeker_id=${jobseekerId}`
        )
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      ;(window as any).ProfileSave?.start?.('Saving professional summary...')
      ...
      if (!data.success) {
        ...
        ;(window as any).ProfileSave?.error?.(data.message || 'Failed to save summary.')
        return
      }
      ...
      const normalizedInitial = new Set(
        initialSkills.map((s) => s.trim()).filter(Boolean)
      )
      const toAdd = normalizedCurrent.filter((s) => !normalizedInitial.has(s))
      const toRemove = Array.from(normalizedInitial).filter(
      ...
      setIsEditing(false)
      ;(window as any).ProfileSave?.success?.('Professional summary saved.')
    } catch (error) {
      console.error('Error saving professional summary:', error)
      ;(window as any).ProfileSave?.error?.('Failed to save summary.')
    } finally {
      setSaving(false)
    }
  }
      ...
      <CardHeader className="bg-gradient-to-r from-emerald-50 to-green-50 rounded-t-lg">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            className={
              isEditing
                ? 'border-red-200 text-white hover:bg-red-50 hover:text-red-500'
      ...
      <CardContent className="p-6">
        {loading ? (
          <div className="flex items-center justify-center py-12">
            ...
            <span className="ml-3 text-gray-600">Loading professional summary...</span>
          </div>
        ) : (
