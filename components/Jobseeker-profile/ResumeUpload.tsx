...
export default function ResumeUpload() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Simulated progress UI while upload is in-flight
      ...
      <CardHeader className="bg-gradient-to-r from-emerald-50 to-green-50 rounded-t-lg">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <CardDescription className="text-emerald-600">Upload and manage your resume files</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="p-6">
        <div className="space-y-6">
          <div className="border-2 border-dashed border-emerald-300 rounded-lg p-8 text-center bg-emerald-50/50">
            ...
            <p className="text-sm text-gray-500 mb-4">Supported formats: PDF, DOC, DOCX (Max size: 5MB)</p>
            ...
            <label htmlFor="resume-upload">
              <Button className="bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white" disabled={isUploading} asChild>
          ...
          <div className="space-y-4">
            ...
            {resumes.map(resume => (
              <Card key={resume.id} className={`border transition-colors ${resume.status === 'active' ? 'border-emerald-300 bg-emerald-50/50' : 'border-gray-200 hover:border-emerald-200'}`}>
